...
```{r}
# wildfire linear model 
wildfire_lm <- lm(data = wildfires_data, n_fires ~ mean_vpd_kpa)
```
...
The coefficient for the `mean_vpd_kpa` is significant in the poisson model but not in the linear model.
