...
QUESTION: HOW TO PIVOT? HOW DO YOU ASSIGN BOTH PROP AND N TO THE FILL THE VALUES?
...
A bar plot aggregates data and creates bars based on specific categorical data. Histograms plot continuous data as frequencies. (Binning data is completed in the histogram graph.)
...
```{r}
# Reading in harmful algal bloom data from a downloaded ncei.noaa.gov web
algal_bloom <- read_csv(here::here("week1", "harmful-algal-blooms", "data", "habsos_20240430.csv"))
```

```{r}
# Cleaning the data 
algal_bloom <- janitor::clean_names(algal_bloom)

```

## Exploring harmful algal bloom data
...
```{r}
algal_bloom |> 
  filter(cellcount >= 100) |> 
  count(cellcount) |> 
  ggplot(aes(cellcount, n)) +
  geom_point() 
...
algal_bloom |> 
  filter(cellcount >= 100) |> 
  group_by(state_id) |> 
  summarize(mean_cellcount = mean(cellcount)) |> 
  ggplot() +
  geom_bar(aes(mean_cellcount))
```
...
```{r}
algal_bloom |> 
  mutate(cellcount_log = log(cellcount + 1)) |> 
   filter(cellcount != 0) |> 
  ggplot(aes(cellcount_log)) +
  geom_histogram(bins = 30) +
  labs(title = "Harmful Algal Bloom (HAB), Karenia brevis, Logarithmic Cell Count",
       subtitle = "Excluding zeros",
      x = "Log of Karenia brevis Cell Count",
      y = "Frequency") +
  theme_classic()
```
...
Creating a histogram showing the algal bloom counts

```{r}
algal_bloom |> 
  mutate(cellcount_log = log(cellcount)) |> 
  count(cellcount) |> 
  ggplot(aes(cellcount_log)) +
  geom_histogram(bin = 0.5) +
  labs(title = "Algal Bloom Cell Count",
       x = "Algal Bloom Cell Count",
       y = "Frequency") +
  theme_classic()
```
