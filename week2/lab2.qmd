...
# `beta_0` is the intercept in estimate in this case: 3.9133
# `beta_1` is the slope in this case: 2.1697
# `sigma` is the residual stantard error in this case: 2.32
...
Hat because these are estimates
...
y_hat <- 3.9 + 2.2 * x #WHY ARE WE DEFINING THIS? 
...
y are observed gage heights

y bar is the mean gage height (overall)

y hat is the expected gage height (given precip)
...
Half of the variance has been removed.
...
mu <- beta_0 +beta_1 * x
y <- rnorm(n = 100, mean = mu, sd = sigma) # WHY DO WE SET n TO EQUAL 100?
...
# Alternative way of calling beta_0 and beta_1 from the mod_summ list
# beta_0_hat <- mod_summ$coefficients["x", "Estimate"]
```
