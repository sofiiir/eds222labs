...
The appropriate test statistic is difference in proportions.

### State your null and alternative hypotheses

**Q2: What are your null and alternative hypotheses?**

H0: There is no difference in successes and failures between garden staples and popsicle sticks.

HA: Popsicle sticks are more successful compared to garden staples.
...
diff <- restoration_summary$prop_success[2]- restoration_summary$prop_success[1]
```
...
#| label: q3
# Group by treatment
rest_stats <- restoration |> 
  group_by(treatment) |> 
  summarise(treat_suc = sum(success), # sum successes
            treat_prop = treat_suc/n()) # Divide by total number

# Difference between garden staples and popsicle sticks 
# The HA is that PS are better so it goes first
 rest_stats$treat_prop[2] - rest_stats$treat_prop[1]
...
#   summarise(treat_suc = n()) |> 
#   mutate(treat_prop = treat_suc/sum(treat_suc))
```
...
p_val <- sum(null_distribution >= diff)/1000
